  ...
  async function handleKeypress(e) {
    if (e.key === "Enter") {
      ...
      pokepromise = getPokemon(
        "https://pokeapi.co/api/v2/pokemon/".concat(pokemon_name.toLowerCase()),
      );
    }
  }
</script>

<h1 class="align-text-top">Pokédex</h1>

<div>
  <div>
    <input
      bind:value={pokemon_name}
      placeholder="enter pokemon name"
      on:keypress={handleKeypress}
      class="h-6 content-center top"
    />
  </div>
  {#await pokepromise then pokemon}
    <div class="content-center">
      {#if selectedPokemon}
        <PokeSummary {pokemon} />
        <PokeStats {pokemon} />
        <PokeMoves {pokemon} method="level-up" version="scarlet-violet" />
      {:else}
        Select a pokemon
      {/if}
    </div>
